...
TEST(ParrotTest, SpeedOfEuropeanParrot) {
    auto parrot = Parrot::create_parrot(ParrotType::EUROPEAN, 0, 0, false);
...
TEST(ParrotTest, SpeedNorwegianBlueParrot_nailed) {
    auto parrot = Parrot::create_parrot(ParrotType::NORWEGIAN_BLUE, 0, 1.5, true);
    EXPECT_NEAR(0.0, parrot->getSpeed(), 0.0);
}
...
TEST(ParrotTest, CryNorwegianBlueParrot_high_voltage) {
    auto parrot = Parrot::create_parrot(ParrotType::NORWEGIAN_BLUE, 0, 24, true);
